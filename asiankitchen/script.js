...
let funcForButtons = () => {
    ...
    let btnContainerDOM = document.querySelector(".btn-container"); // buttonların bulundugu alan

    let buttons = allCategories.map(buttonName => { //buttonları allCategories arrayından çekmek.
        return `<button class="btn btn-outline-dark btn-item" data-id=${buttonName}>${buttonName}</button>`; // buttonları oluşturmak
    });
    buttons = buttons.join(""); // buttonların arasındaki "," kaldırmak
    btnContainerDOM.innerHTML = buttons; // buttonları eklemek
}
...
let funcForItems = () => {
    ...
    let items = menu.map(index => { // menu'nun altındaki objelere ulaşarak, objedeki values'lerini key'lerini kullanarak ulaşmak.
        return `
    ...
    //filter işlemlerin başlangıcı
    let chinaArr = []; //yukaırda oluşyurudugmuz divleri classlarına gore arrayların ıcınde tutmak ıcın 
    let chinaDOM = document.querySelectorAll(".china");
    chinaDOM.forEach(i => {
        chinaArr.push(i); // array div'leri pushlamak
    })

    let japanArr = []; //yukaırda oluşyurudugmuz divleri classlarına gore arrayların ıcınde tutmak ıcın 
    let japanDOM = document.querySelectorAll(".japan");
    japanDOM.forEach(i => {
        japanArr.push(i); // array div'leri pushlamak
    });

    let koreaArr = []; //yukaırda oluşyurudugmuz divleri classlarına gore arrayların ıcınde tutmak ıcın 
    let koreaDOM = document.querySelectorAll(".korea");
    koreaDOM.forEach(i => {
        koreaArr.push(i); // array div'leri pushlamak
    });

    let buttonsDOM = document.querySelectorAll(".btn"); // oluşan buttonlara ulaşmak.
    

    buttonsDOM.forEach(index => {
        index.addEventListener("click", event => { // buttonlara click eventi eklemek
            let data = event.target.dataset.id; // buttonların datasetlerine ulaşmak
            
            if (data == "China") { // yukarıda ulaştıgımız datasetleri kullanarak kontrol yapıp html içerisinde değişiklik yapmak
            ...
            if (data == "ALL") {
                sectionCenterDOM.innerHTML = items // butun itemlerı gostermek
            }
